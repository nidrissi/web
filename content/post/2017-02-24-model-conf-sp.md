...
$$\mathtt{G}_A(k) = \bigl( A^{\otimes k} \otimes S(\omega_{ij}) / \text{relations}, d \omega_{ij} = [\Delta_{ij}].$$
...
**Theorem.** Let $$M$$ be a smooth, simply connected manifold of dimension at least $$4$$. Then $$\mathtt{G}_(k)$$ is a *real* model for $$\Conf_k(M)$$, for all $$k \ge 0$$.
